...
function Start()
    ...
    ProjectileSpeed = 0.8
    ProjectileRigidBody = API_GameObject.BL_GetComponent2(BL_Host,"Rigidbody")
    Webline = API_GameObject.BL_GetComponent2(BL_Host,"LineRenderer")
    --RopeJoint = API_GameObject.BL_GetComponent2(BL_Host,"ConfigurableJoint")
...
function OnCollisionEnter(collision)
    if(not Frozen and collision.collider.gameObject.transform.root ~= OwnerGun.transform.root) then
        FreezeProjectile()
...
function FreezeProjectile()
    ...
    --disable auto configured connected offset dealie, set to 0,0,0 or it won't work

    --marrowJoint =API_GameObject.BL_GetComponent2(BL_Host,"MarrowJoint")
   -- marrowEntity = API_GameObject.BL_GetComponent2(BL_Host,"MarrowEntity")

  --  marrowJoint._bodyA =  API_GameObject.BL_GetComponent2(BL_Host,"MarrowBody")
    --marrowJoint._bodyB =  API_GameObject.BL_GetComponent2(OwnerGun.gameObject,"MarrowBody")
  --  marrowJoint._entity = marrowEntity
  --  marrowJoint.CopyJointInfo(RopeJoint)
  --  marrowJoint.Validate(RopeJoint,marrowEntity)
...
function SetOwnerGun(owner)
    ...
    OwnerGun = owner
    OwnerGunRB = API_GameObject.BL_GetComponent2(OwnerGun.gameObject,"Rigidbody")
end

...
namespace LuaMod
{
    public class LuaResources : MonoBehaviour
    {
        ...
        public static T Cast<T>(UnityEngine.Object input) where T : class
        {
        ...
        public DynValue GetObject(string key, string CompType)
        {
            ...
            {
                if (!Objects.TryGetValue(key, out UnityEngine.Object obj) || obj == null)
                {
                    throw new ScriptRuntimeException($"Object with key '{key}' not found");
                }
                    

                return API_Utils.BL_ConvertObjectToType(obj, CompType);
